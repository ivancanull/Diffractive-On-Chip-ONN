...
class DiffractiveLayer(tf.keras.layers.Layer):
    ...
    def call(self, x, waves):
        ...
        # dummy
        if self.dummy:
            waves = tf.multiply(waves, self.mask)
    ...
    def initialize_dummy_masks(self):
        self.mask.assign(tf.ones_like(self.x0, dtype=tf.complex64))

    def remove_overlapping(self):
        self.initialize_dummy_masks()
        le = self.x0[0] + self.x0_bound
        for i in range(self.neuron_number - 1):
            ns = self.x0[i + 1] - self.x0_bound
            ne = self.x0[i + 1] + self.x0_bound
            if ns > le:
                le = ne
            else:
